...
def wsc(X, y, S, delta=0.1, M=1000, random_state=2020, verbose=False):
    ...
    if verbose:
        ...
        for m in range(M):
            wsc_list[m], a_list[m], b_list[m] = wsc_v(X, y, S, delta, V[m])
    # np.save('wsc_list_aps_focalloss4_226_4897.npy', np.array(wsc_list))
    # exit(0)
...
def wsc_unbiased(X, y, S, delta=0.1, M=1000, test_size=0.75, random_state=2020, verbose=False):
    ...
    X_train, X_test, y_train, y_test, S_train, S_test = train_test_split(X, y, S, test_size=test_size, random_state=random_state)
    # wsc_star, v_star, a_star, b_star = wsc(X, y, S, delta=delta, M=M, random_state=random_state, verbose=verbose)
